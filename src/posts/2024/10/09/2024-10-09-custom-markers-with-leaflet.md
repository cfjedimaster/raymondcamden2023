...
Out of the box, this is it. Period. I can appreciate the library wanting to keep its size to a minimum, but I was a bit surprised. That being said, the library provides really flexible support for creating your own markers. The first thing I found was the tutorial, [Markers With Custom Icons](https://leafletjs.com/examples/custom-icons/). In this tutorial, they describe the process of creating an instance of the `icon` class and specifying resources for the icon and shadow. Technically, this looked incredibly simple, and again, flexible, but the immediate issue I ran into was... ok, where do I find more icons?

I did some more digging and came across this open-source project, [leaflet-color-markers](https://github.com/pointhi/leaflet-color-markers), which provides a set of markers in the same style as the original, but in a variety of colors. Their read.me demonstrates the usage like so:
